...
func (r Monitoring) ListSamples(ctx context.Context, stationID int64, page domain.Page) ([]domain.Sample, error) {
	rows, err := r.DB.QueryContext(ctx, `SELECT id,station_id,sampled_at,quality_class,metrics_json,created_by,created_at FROM samples WHERE station_id=? ORDER BY sampled_at DESC LIMIT ? OFFSET ?`, stationID, page.Limit(), page.Offset())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
