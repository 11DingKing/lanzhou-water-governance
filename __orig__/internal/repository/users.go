...
func (r Users) Session(ctx context.Context, token string, now time.Time) (domain.User, error) {
	...
	err := r.DB.QueryRowContext(ctx, `SELECT u.id,u.username,u.role,u.region,u.disabled,u.created_at,s.expires_at FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.revoked_at IS NULL`, token).Scan(&u.ID, &u.Username, &u.Role, &u.Region, &disabled, &created, &expiry)
	if err == sql.ErrNoRows {
