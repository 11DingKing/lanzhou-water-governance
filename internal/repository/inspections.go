...
func (r Inspections) Transition(ctx context.Context, id int64, from, to domain.InspectionStatus, version int64, notes string) (domain.Inspection, error) {
	...
	res, err := r.DB.ExecContext(ctx, `UPDATE inspections SET status=?,started_at=COALESCE(started_at,?),completed_at=?,notes=?,version=version+1 WHERE id=? AND status=?`, to, start, complete, notes, id, from, version)
	if err != nil {
