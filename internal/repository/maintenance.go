...
func (r Maintenance) DeleteOldSamples(ctx context.Context, cutoff time.Time) (int64, error) {
	res, err := r.DB.ExecContext(ctx, `DELETE FROM samples WHERE sampled_at < ? AND NOT EXISTS(SELECT 1 FROM alerts WHERE alerts.sample_id=samples.id)`, cutoff.Format(time.RFC3339Nano))
	if err != nil {
		return 0, err
	}
	return res.RowsAffected()
