...
func TestB12BidirectionalWarningRetry(t *testing.T) {
    _ = testsupport.Open; original:=map[string]any{"direction":"downstream"};copy:=domain.CloneMap(original);copy["direction"]="upstream";if original["direction"]!="downstream"{t.Fatalf("payload was shared: %v",original)}
}
