...
func TestD13WorkflowRollbackEvidence(t *testing.T) {
    _ = domain.RoleAdmin; db:=testsupport.Open(t);sentinel:=errors.New("rollback evidence");err:=sqlite.WithTx(context.Background(),db.SQL,func(*sql.Tx)error{return sentinel});if !errors.Is(err,sentinel){t.Fatalf("transaction error was swallowed: %v",err)}
}
