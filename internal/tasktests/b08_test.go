...
func TestB08WorkerFailurePropagation(t *testing.T) {
    _ = domain.RoleAdmin; _ = testsupport.Open; runner:=worker.Runner{MaxAttempts:2,Interval:time.Millisecond};err:=runner.ExecuteForTest(context.Background(),func(context.Context)error{return errors.New("temporary")});if err==nil{t.Fatalf("exhausted worker failure was acknowledged")}
}
