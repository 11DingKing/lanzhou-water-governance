...
func (s Waste) Advance(ctx context.Context, user domain.User, id int64, from, to domain.ManifestStatus, version int64) (domain.Manifest, error) {
	...
	phase := manifestPhase(to)
	_ = s.Audit.Record(ctx, user.ID, requestID(ctx), "manifest", fmt.Sprint(id), phase, "ok", nil)
...
func manifestPhase(status domain.ManifestStatus) string { return string(status) }
