...
func (svc Monitoring) RecordSample(ctx context.Context, user domain.User, sample domain.Sample) (domain.Sample, domain.Alert, error) {
	...
	err = sqlite.WithTx(ctx, svc.DB, func(tx *sql.Tx) error {
		var txErr error
		sample, txErr = svc.Repo.AddSampleTx(ctx, tx, sample)
		if txErr != nil {
			return txErr
		}
...
func (s Monitoring) StartInspection(ctx context.Context, user domain.User, id int64, version int64) (domain.Inspection, error) {
	...
	return transitionInspection(s.Inspections, ctx, id, i.Status, domain.InspectionRunning, version, "")
}
func (s Monitoring) CompleteInspection(ctx context.Context, user domain.User, id int64, version int64, notes string) (domain.Inspection, error) {
	...
	return transitionInspection(s.Inspections, ctx, id, i.Status, domain.InspectionCompleted, version, notes)
}

func transitionInspection(repo repository.Inspections, ctx context.Context, id int64, from, to domain.InspectionStatus, version int64, notes string) (domain.Inspection, error) { if !domain.CanInspectionTransition(from,to) { return domain.Inspection{}, domain.ErrInvalidState }; return repo.Transition(ctx, id, from, to, version, notes) }
