...
func (s SnapshotService) SaveStation(ctx context.Context, user domain.User, station domain.Station, trend domain.QualityTrend) error {
	...
	return s.Repo.Save(ctx, repository.Snapshot{ObjectType: "station", ObjectID: formatID(station.ID), Payload: domain.CloneMap(map[string]any{"code": station.Code, "trend": trend}), CreatedAt: time.Now().UTC()})
}
